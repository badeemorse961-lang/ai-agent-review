...
def write_registry_fixture(tmp_path: Path, *, groq_status: str = "ACTIVE", groq_active: bool = True) -> tuple[Path, Path, Path, Path, Path, Path]:
    ...
    connections = {
        ...
        "connections": {
            "OR-01": {
                "connection_id": "OR-01", "provider": "openrouter", "key_fingerprint": hashlib.sha256(b"or-secret").hexdigest(),
                "role": None, "status": "ACTIVE", "active": True, "credential_validated": True, "validation_required": False,
            },
            **{
                f"GROQ-{i:02d}": {
                    "connection_id": f"GROQ-{i:02d}", "provider": "groq", "key_fingerprint": hashlib.sha256(f"groq-{i}".encode()).hexdigest(),
                    "role": None, "status": groq_status if i == 1 else "ACTIVE", "active": groq_active if i == 1 else True,
    ...
    leader_health.write_text(json.dumps({"provider": "openrouter", "models": {"m1": {"provider": "openrouter", "healthy": ["OR-01"], "failed": []}, "m2": {"provider": "openrouter", "healthy": ["OR-01"], "failed": []}}}), encoding="utf-8")
...
def test_leader_router_excludes_disabled_connection(tmp_path: Path) -> None:
    ...
    with patch("leader_router.REGISTRY_FILE", registry), patch("config_registry.REGISTRY_FILE", registry), patch("config_registry.CONNECTIONS_FILE", connections):
...
def test_worker_router_excludes_failed_invalid_and_removed_connections(tmp_path: Path) -> None:
    for lifecycle in ("FAILED", "INVALID", "REMOVED"):
        ...
        with patch("config_registry.REGISTRY_FILE", registry), patch("config_registry.CONNECTIONS_FILE", connections):
