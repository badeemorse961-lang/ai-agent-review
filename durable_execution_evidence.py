...
class EvidenceLayer:
    ...
    def create_checkpoint(self, *, project_id: str, run_id: str, phase_id: str, task_id: str, attempt_id: str, sequence: int, checkpoint_kind: str, workspace_evidence_identity: str, workspace_evidence_hash: str, checkpoint_id: str) -> CheckpointRecord:
        ...
        with self.state._transaction() as conn:
            ...
            if not any((item["observed_after_identity"] or item["expected_after_identity"]) == workspace_evidence_identity and self._evidence_row_digest(item) == workspace_evidence_hash for item in evidence_rows):
            ...
            if existing is not None:
                fields = {"project_id": project_id, "run_id": run_id, "phase_id": phase_id, "task_id": task_id, "attempt_id": attempt_id, "sequence": sequence, "checkpoint_kind": checkpoint_kind, "workspace_evidence_identity": workspace_evidence_identity, "workspace_evidence_hash": workspace_evidence_hash}
                if any(existing[key] != value for key, value in fields.items()):
                    raise LineageError("Checkpoint identity already maps to different lineage/evidence")
    ...
    def transition_checkpoint(self, *, project_id: str, run_id: str, checkpoint_id: str, new_status: str) -> CheckpointRecord:
        ...
        with self.state._transaction() as conn:
            ...
            evidence_rows = conn.execute("SELECT * FROM workspace_evidence WHERE project_id=? AND run_id=? AND phase_id=? AND task_id=? AND attempt_id=?", (project_id, run_id, row["phase_id"], row["task_id"], row["attempt_id"])).fetchall()
            if not any((item["observed_after_identity"] or item["expected_after_identity"]) == row["workspace_evidence_identity"] and self._evidence_row_digest(item) == row["workspace_evidence_hash"] for item in evidence_rows):
    ...
    def register_artifact(self, *, project_id: str, run_id: str, phase_id: str, task_id: str, attempt_id: str, artifact_id: str, reference: str, identity: str, checksum: Optional[str] = None, state: str = "ARTIFACT_UNVALIDATED") -> ArtifactRecord:
        if not reference or not identity or state not in ARTIFACT_STATES or state != "ARTIFACT_UNVALIDATED":
            raise IntegrityError("Artifact registration requires reference, identity and ARTIFACT_UNVALIDATED state")
        with self.state._transaction() as conn:
            self.state._require_attempt(conn, project_id, run_id, phase_id, task_id, attempt_id)
            ...
            if existing is not None:
                if any(existing[key] != value for key, value in {"project_id": project_id, "run_id": run_id, "phase_id": phase_id, "task_id": task_id, "attempt_id": attempt_id, "reference": reference, "identity": identity, "checksum": checksum}.items()):
                    raise LineageError("Artifact identity already maps to different lineage/evidence")
                return self._artifact_record(existing)
            run = self.state._require_run(conn, project_id, run_id)
            next_sequence = int(run["sequence"]) + 1
            conn.execute("INSERT INTO artifacts(artifact_id, project_id, run_id, phase_id, task_id, attempt_id, reference, identity, checksum, state, created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)", (artifact_id, project_id, run_id, phase_id, task_id, attempt_id, reference, identity, checksum, state, utc_now()))
            self.state._append_event_tx(conn, project_id=project_id, run_id=run_id, sequence=next_sequence, event_type="ARTIFACT_REGISTERED", entity_type="artifact", entity_id=artifact_id, payload={"phase_id": phase_id, "task_id": task_id, "attempt_id": attempt_id, "state": state})
            conn.execute("UPDATE runs SET sequence=? WHERE project_id=? AND run_id=?", (next_sequence, project_id, run_id))
    ...
    def create_validation(self, *, project_id: str, run_id: str, phase_id: str, task_id: str, attempt_id: str, checkpoint_sequence: int, validation_id: str, artifact_id: Optional[str] = None) -> ValidationRecord:
        with self.state._transaction() as conn:
            ...
            if existing is not None:
                ...
                if existing["artifact_id"] != artifact_id or int(existing["checkpoint_sequence"]) != checkpoint_sequence:
                    raise LineageError("Validation identity already maps to different evidence")
            ...
            self.state._append_event_tx(conn, project_id=project_id, run_id=run_id, sequence=next_sequence, event_type="VALIDATION_CREATED", entity_type="validation", entity_id=validation_id, payload={"phase_id": phase_id, "task_id": task_id, "attempt_id": attempt_id, "artifact_id": artifact_id, "checkpoint_sequence": checkpoint_sequence})
            conn.execute("UPDATE runs SET sequence=? WHERE project_id=? AND run_id=?", (next_sequence, project_id, run_id))
    ...
    def bind_lease(self, *, project_id: str, run_id: str, phase_id: str, task_id: str, attempt_id: str, lease_id: str, worker_id: Optional[str]) -> LeaseBindingRecord:
        with self.state._transaction() as conn:
            ...
            next_sequence = int(run["sequence"]) + 1
            now = utc_now()
            conn.execute("INSERT INTO lease_bindings(lease_id, project_id, run_id, phase_id, task_id, attempt_id, worker_id, bound_at, released_at, state) VALUES(?,?,?,?,?,?,?,?,?,?)", (lease_id, project_id, run_id, phase_id, task_id, attempt_id, worker_id, now, None, "ACTIVE"))
            ...
            self.state._append_event_tx(conn, project_id=project_id, run_id=run_id, sequence=next_sequence, event_type="LEASE_BOUND", entity_type="lease", entity_id=lease_id, payload={"phase_id": phase_id, "task_id": task_id, "attempt_id": attempt_id, "worker_id": worker_id})
            conn.execute("UPDATE runs SET sequence=? WHERE project_id=? AND run_id=?", (next_sequence, project_id, run_id))
        ...
        with self.state._transaction() as conn:
            ...
            if existing is not None:
                if any(existing[key] != value for key, value in {"project_id": project_id, "run_id": run_id, "phase_id": phase_id, "task_id": task_id, "attempt_id": attempt_id, "operation_kind": operation_kind}.items()):
                    raise LineageError("Idempotency key already maps to different recovery context")
    ...
    def transition_recovery_operation(self, *, project_id: str, run_id: str, operation_id: str, new_state: str, effect_reference: Optional[str] = None, effect_hash: Optional[str] = None) -> RecoveryOperationRecord:
        if new_state not in _RECOVERY_STATES:
...
__all__ = ["ArtifactRecord", "CheckpointRecord", "EvidenceLayer", "LeaseBindingRecord", "RecoveryOperationRecord", "ValidationRecord", "evidence_digest"]