...
The control plane does not expose push, pull, fetch, reset, clean, checkout, switch, restore, merge, rebase, cherry-pick, stash, tag, remote, worktree, config injection, repository-scope overrides, `--amend`, or `--no-verify`.
...
- any working-tree change exists outside the authorized target set;
