...
def _fixture(tmp_path: Path, *, artifact_state: str = "ARTIFACT_VALIDATED", delivery_state: str = "DELIVERY_PENDING"):
    ...
    task = {"task_id": "TASK-1", "objective": "workspace recovery", "retry_class": "IDEMPOTENT", "depends_on": []}
    ...
    identity = "sha256:" + hashlib.sha256(b"v1").hexdigest()
    artifact = EvidenceLayer(state).register_artifact(project_id=bridge.project_id, run_id=run_id, phase_id=phase_id, task_id="TASK-1", attempt_id=attempt_id, artifact_id="ART-1", reference="artifact.txt", identity=identity, checksum=identity.removeprefix("sha256:"))
    evidence = WorkspaceEvidence(bridge.project_id, run_id, phase_id, "TASK-1", attempt_id, "artifact.txt", "CREATED", None, None, identity, identity, "PRESENT_COMPLETE", None, "ART-1", "2026-09-09T00:00:00+00:00")
    evidence_id = EvidenceLayer(state).record_workspace_evidence(evidence)
    checkpoint = EvidenceLayer(state).create_checkpoint(project_id=bridge.project_id, run_id=run_id, phase_id=phase_id, task_id="TASK-1", attempt_id=attempt_id, sequence=1, checkpoint_kind="WORKSPACE", workspace_evidence_identity=identity, workspace_evidence_hash=evidence_digest(evidence), checkpoint_id="CP-1")
    ...
    validation = EvidenceLayer(state).create_validation(project_id=bridge.project_id, run_id=run_id, phase_id=phase_id, task_id="TASK-1", attempt_id=attempt_id, checkpoint_sequence=1, validation_id="VAL-1", artifact_id="ART-1")
    ...
    if artifact_state == "ARTIFACT_VALIDATED":
        EvidenceLayer(state).validate_artifact(project_id=bridge.project_id, run_id=run_id, artifact_id="ART-1", observed_identity=identity, observed_checksum=identity.removeprefix("sha256:"))
    ...
    return root, state, bridge, run_id, phase_id, "TASK-1", attempt_id, path, identity, evidence_id


def test_exact_match_preserves_validated_artifact(tmp_path: Path):
    root, state, bridge, run_id, phase_id, task_id, attempt_id, path, identity, _ = _fixture(tmp_path)
...
def test_validated_artifact_modified_after_validation_requires_revalidation(tmp_path: Path):
    root, state, bridge, run_id, phase_id, task_id, attempt_id, path, _, _ = _fixture(tmp_path)
...
def test_ambiguous_reference_is_recovery_required(tmp_path: Path):
    root, state, bridge, run_id, phase_id, task_id, attempt_id, *_ = _fixture(tmp_path)
    state._connection.execute("UPDATE artifacts SET reference='not-a-file-or-directory-name' WHERE artifact_id='ART-1'")
    decision = WorkspaceDeliveryRecovery(state, root).inspect_artifact(project_id=bridge.project_id, run_id=run_id, phase_id=phase_id, task_id=task_id, attempt_id=attempt_id, artifact_id="ART-1")
    assert decision.classification == "ABSENT"
...
def test_delivery_recovery_transitions_started_to_recovery_required(tmp_path: Path):
    ...
    assert state._connection.execute("SELECT delivery_state FROM runs WHERE run_id=?", (run_id,)).fetchone()[0] == "DELIVERY_RECOVERY_REQUIRED"
...
def test_delivery_committed_is_not_verified_automatically(tmp_path: Path):
    ...
    assert state._connection.execute("SELECT delivery_state FROM runs WHERE run_id=?", (run_id,)).fetchone()[0] == "DELIVERY_COMMITTED"
...
def test_cross_project_run_isolation(tmp_path: Path):
    ...
    other_root.mkdir()
    other_project = _project_id(other_root)
    state.create_project(other_root, other_project)
    other_run = state.create_run(other_project)
