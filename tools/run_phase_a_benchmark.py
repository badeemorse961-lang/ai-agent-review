...
def _failure_category(result: RunResult) -> str:
    error_name = str(result.error or "").split(":", 1)[0].strip()
    if not error_name:
        return "none"
...
def _connection_summary(items: Sequence[RunResult]) -> dict[str, Any]:
    summary = dict(_aggregate_group(list(items)))
    dimensions = summary.get("dimensions")
    if isinstance(dimensions, Mapping):
        summary["dimensions_reliability"] = _safe_number(dimensions.get("reliability"))
    return summary
...
def model_level_aggregate(results: Sequence[RunResult]) -> dict[str, Any]:
    """Compare model capabilities using an equal-weight macro-average per connection."""
...
def model_workload_aggregate(results: Sequence[RunResult]) -> dict[str, Any]:
    ...
    return {
        f"{provider}:{model}:{task_class}": _connection_summary(items)
...
def connection_level_aggregate(results: Sequence[RunResult]) -> dict[str, Any]:
    ...
    for (provider, connection_id), items in sorted(groups.items()):
        ...
        repeated_failures = sum(max(0, count - 1) for count in signatures.values())
        attempts = len(items)
...
def build_phase_a_payload(
    *,
    ...
    return {
        ...
        "aggregate": benchmark.aggregate(list(results)),
...
def main() -> int:
    ...
    parser.add_argument("--phase-a", action="store_true", help="Discover all locally eligible connections and run the existing Phase A model shortlist.")
    parser.add_argument("--inventory-only", action="store_true", help="Discover and print inventory without invoking providers.")
    parser.add_argument("--provider", action="append", choices=["openrouter", "groq"], help="Limit discovered providers; repeatable.")
    ...
    payload_inventory = inventory_summary(inventory)
    payload_inventory["benchmark_models"] = {
        "openrouter": list(benchmark.OPENROUTER_CANDIDATES),
        "groq": list(benchmark.GROQ_CANDIDATES),
    }
    payload_inventory["candidate_count"] = len(candidates)
    payload_inventory["candidate_matrix"] = [candidate.__dict__ for candidate in candidates]
    print(json.dumps(payload_inventory, indent=2, ensure_ascii=False))
    ...
    results = run_benchmark(root, candidates, args.repeats, set(args.classes) if args.classes else None)
