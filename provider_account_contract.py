...
def _require_string_sequence(value: Any, field: str) -> tuple[str, ...]:
    ...
    if not isinstance(value, (list, tuple)):
        raise ValueError(
            f"ProviderAccount {field} must be a sequence of strings, got {type(value).__name__}"
        )
    normalized: list[str] = []
    for item in value:
        if not isinstance(item, str):
            raise ValueError(
                f"ProviderAccount {field} contains non-string: {type(item).__name__}"
            )
        stripped = item.strip()
        if not stripped:
...
class ProviderAccountService:
    ...
    def resolve_account(
        ...
        if active_item is not None:
            ...
            if not isinstance(provider, str) or not provider.strip():
                raise ValueError(
                    f"Active pool account {connection_id} has no provider metadata"
                )
