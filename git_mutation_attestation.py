    ...
    def __post_init__(self) -> None:
        ...
        for name, value in {
            ...
            if not isinstance(value, str) or not value.strip():
                raise GitMutationAttestationError(f"Attestation {name} must be non-empty")
            if any(char not in "0123456789abcdefABCDEF" for char in value):
                raise GitMutationAttestationError(f"Attestation {name} must be hexadecimal")
...
def attestation_from_dict(value: Mapping[str, Any]) -> GitMutationAttestation:
    ...
    if value.get("schema_version") != SCHEMA_VERSION:
        raise GitMutationAttestationError("Unsupported transaction-attestation schema version")
    targets = value.get("targets")
    if not isinstance(targets, list):
        raise GitMutationAttestationError("Attestation targets must be serialized as a list")
