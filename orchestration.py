...
class WorkerExecutionSpec:
    """Legacy execution view retained for compatibility with existing tests.

    New worker adapters should return ``WorkerWorkProduct``. The orchestrator
    converts it to this narrow execution view immediately before the existing
    execution boundary, preserving the established authority semantics.
    """
...
class WorkerAdapter(Protocol):
    ...
    ) -> WorkerWorkProduct | WorkerExecutionSpec: ...
            ...
            "task_records": [
                {
                    ...
                    "work_product": dict(record.work_product),
                }
...
class CanonicalOrchestrator:
    """Compose the validated Core boundaries into the canonical flow."""
    ...
    def __init__(
        ...
        *,
        ...
        allow_legacy_worker_spec: bool = True,
    ) -> None:
        ...
        self.allow_legacy_worker_spec = bool(allow_legacy_worker_spec)
    ...
    def run(self, task_id: str) -> OrchestrationResult:
        ...
        try:
            ...
            for task in self._ordered_tasks(tasks):
                ...
                raw_product = self.worker_adapter.prepare(task, assignment_dict)
                work_product, spec = self._normalize_worker_output(
                    raw_product,
                    task=task,
    ...
    def _normalize_worker_output(
        self,
        raw: WorkerWorkProduct | WorkerExecutionSpec,
        *,
        task: Mapping[str, Any],
        assignment: Mapping[str, Any],
    ) -> tuple[WorkerWorkProduct, WorkerExecutionSpec]:
        task_id = task.get("task_id")
        role = task.get("role")
        worker_id = assignment.get("worker_id")
        if not all(isinstance(item, str) and item.strip() for item in (task_id, role, worker_id)):
            raise OrchestrationSafetyStop("Worker task/role/worker identity is invalid")
        if isinstance(raw, WorkerWorkProduct):
            try:
                raw.validate(task_id=task_id, worker_id=worker_id, role=role)
            except WorkerWorkProductError as exc:
                raise OrchestrationSafetyStop(str(exc)) from exc
            if raw.status == "FAILED":
                reason = raw.failure.get("reason", "unspecified") if raw.failure else "unspecified"
                raise OrchestrationSafetyStop(
                    f"Worker work-product reported failure for {task_id!r}: {reason}"
                )
            return raw, raw.to_execution_spec()
        if not self.allow_legacy_worker_spec:
            raise OrchestrationSafetyStop(
                "Production worker adapters must return WorkerWorkProduct; legacy WorkerExecutionSpec is not permitted"
            )
        try:
            product = WorkerWorkProduct(
                task_id=task_id,
                worker_id=worker_id,
                role=role,
                command=raw.command,
                targets=raw.targets,
                changed_targets=raw.changed_targets,
                changes=raw.changes,
                validation_evidence={},
            )
            product.validate(task_id=task_id, worker_id=worker_id, role=role)
        except (WorkerWorkProductError, AttributeError) as exc:
            raise OrchestrationSafetyStop(
                f"Legacy worker output failed protocol normalization: {exc}"
            ) from exc
        return product, raw
