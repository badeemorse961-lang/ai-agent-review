...
class StartupRecovery:
    ...
    def recover(self) -> StartupRecoveryReport:
        ...
        for run_id in run_ids:
            try:
                ...
                raise StartupRecoverySafetyStop(
                    f"Startup recovery stopped for run {run_id!r}: {exc}"
                ) from exc
        return StartupRecoveryReport(
            project_id=self.project_id,
            workspace_root=str(self.workspace_root),
            discovered_runs=tuple(item.run_id for item in decisions),
    ...
    def _validate_project_binding(self, project: Any) -> None:
        ...
        if recorded != self.workspace_root:
            raise StartupRecoverySafetyStop(
                "Durable project is bound to a different workspace"
            )
    ...
    def _run_ids(self) -> tuple[str, ...]:
        with self.state._lock:
            rows = self.state._connection.execute(
                "SELECT run_id FROM runs WHERE project_id=? ORDER BY created_at, run_id",
                (self.project_id,),
            ).fetchall()
    ...
    def _validate_lineage(self, run_id: str) -> None:
        with self.state._lock:
            ...
            phases = conn.execute(
                "SELECT * FROM phases WHERE project_id=? AND run_id=? ORDER BY ordinal, phase_id",
                (self.project_id, run_id),
            ).fetchall()
            for phase in phases:
                self.state._require_phase(conn, self.project_id, run_id, str(phase["phase_id"]))
            tasks = conn.execute(
                "SELECT * FROM tasks WHERE project_id=? AND run_id=? ORDER BY phase_id, task_id",
                (self.project_id, run_id),
            ).fetchall()
            for task in tasks:
                self.state._require_task(conn, self.project_id, run_id, str(task["phase_id"]), str(task["task_id"]))
            attempts = conn.execute(
                "SELECT * FROM attempts WHERE project_id=? AND run_id=? ORDER BY phase_id, task_id, attempt_id",
                (self.project_id, run_id),
            ).fetchall()
            for attempt in attempts:
                ...
                if attempt["lease_id"] is not None:
                    lease = conn.execute(
                        "SELECT * FROM lease_bindings WHERE lease_id=?",
                        (attempt["lease_id"],),
                    ).fetchone()
                    if lease is None:
            ...
            leases = conn.execute(
                "SELECT * FROM lease_bindings WHERE project_id=? AND run_id=? ORDER BY lease_id",
                (self.project_id, run_id),
            ).fetchall()
            for lease in leases:
                self.state._require_attempt(
            ...
            operations = conn.execute(
                "SELECT * FROM recovery_operations WHERE project_id=? AND run_id=? ORDER BY operation_id",
                (self.project_id, run_id),
            ).fetchall()
    ...
    def _recover_run(self, run_id: str) -> Optional[StartupRecoveryDecision]:
        ...
        decision_key = f"startup-recovery:v1:{self.project_id}:{run_id}"
        operation = self.evidence.create_recovery_operation(
            project_id=self.project_id,
            run_id=run_id,
            operation_kind="STARTUP_INTERRUPTED_RUN_DISCOVERY",
            idempotency_key=decision_key,
        )

        if operation.state == "PLANNED":
            self.evidence.transition_recovery_operation(
                project_id=self.project_id,
                run_id=run_id,
                operation_id=operation.operation_id,
                new_state="STARTED",
            )
            operation = self.evidence.transition_recovery_operation(
                project_id=self.project_id,
                run_id=run_id,
                operation_id=operation.operation_id,
                new_state="UNKNOWN",
                effect_reference=self._decision_reference(run_id, reasons),
                effect_hash=self._decision_hash(run_id, reasons),
            )
        elif operation.state in {"STARTED", "EFFECT_OBSERVED"}:
            operation = self.evidence.transition_recovery_operation(
                project_id=self.project_id,
                run_id=run_id,
                operation_id=operation.operation_id,
        ...
        self._reconcile_active_leases(run_id, decisions)
        ...
        if current.state in {"RUNNING", "CREATED", "PLAN_ACCEPTED"}:
            try:
                self.state.transition_run(
                    ...
                    event_type="STARTUP_RECOVERY_STARTED",
                    entity_id=run_id,
                    payload={"decision": "RECOVERY_REQUIRED", "reason_count": len(decisions)},
                )
        ...
        if current.state == "RECOVERING":
            self.state.transition_run(
                ...
                event_type="STARTUP_RECOVERY_REQUIRED",
                entity_id=run_id,
                payload={"decision": "RECOVERY_REQUIRED", "reasons": tuple(sorted(set(decisions)))},
            )
        return StartupRecoveryDecision(
            run_id=run_id,
            project_id=self.project_id,
            state_before=state_before,
            state_after=self.state.get_run(run_id, project_id=self.project_id).state,
            recovery_operation_id=operation.operation_id,
            recovery_operation_key=operation.idempotency_key,
    ...
    def _candidate_state(self, run_id: str, run_state: str) -> tuple[bool, list[str]]:
        with self.state._lock:
            ...
            if run_state in {"CREATED", "PLAN_ACCEPTED"}:
                child_count = conn.execute(
                    "SELECT COUNT(*) FROM tasks WHERE project_id=? AND run_id=? AND state<>?",
                    (self.project_id, run_id, "PENDING"),
                ).fetchone()[0]
                if int(child_count) > 0:
                    reasons.append("CHILD_WORK_STARTED_BEFORE_RUN_TERMINAL")
            lease_count = conn.execute(
                "SELECT COUNT(*) FROM lease_bindings WHERE project_id=? AND run_id=? AND state IN ('ACTIVE','STALE','AMBIGUOUS')",
                (self.project_id, run_id),
            ).fetchone()[0]
            if int(lease_count) > 0:
                reasons.append("NON_TERMINAL_LEASE_PRESENT")
            task_count = conn.execute(
                "SELECT COUNT(*) FROM tasks WHERE project_id=? AND run_id=? AND state IN ('INTERRUPTED','RECOVERY_REQUIRED')",
                (self.project_id, run_id),
            ).fetchone()[0]
            if int(task_count) > 0:
                reasons.append("INTERRUPTED_OR_RECOVERY_REQUIRED_TASK")
            attempt_count = conn.execute(
                "SELECT COUNT(*) FROM attempts WHERE project_id=? AND run_id=? AND state IN ('STARTED','INTERRUPTED','AMBIGUOUS','RECOVERY_REQUIRED')",
                (self.project_id, run_id),
            ).fetchone()[0]
            if int(attempt_count) > 0:
                reasons.append("NON_TERMINAL_ATTEMPT_PRESENT")
            operation_count = conn.execute(
                "SELECT COUNT(*) FROM recovery_operations WHERE project_id=? AND run_id=? AND state IN ('PLANNED','STARTED','EFFECT_OBSERVED','UNKNOWN')",
                (self.project_id, run_id),
            ).fetchone()[0]
    ...
    def _reconcile_active_leases(self, run_id: str, decisions: list[str]) -> None:
        with self.state._lock:
            leases = self.state._connection.execute(
                "SELECT * FROM lease_bindings WHERE project_id=? AND run_id=? AND state='ACTIVE' ORDER BY lease_id",
                (self.project_id, run_id),
            ).fetchall()
        for lease in leases:
            ...
            try:
                result = self.worker_recovery.mark_ambiguous(
                    project_id=self.project_id,
                    run_id=run_id,
                    lease_id=lease_id,
                    idempotency_key=key,
                    ambiguity_reason="Startup cannot prove worker survival or external-effect absence",
                )
