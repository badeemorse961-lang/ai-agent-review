...
class WorkerLeaseRecovery:
    ...
    def bind_actual_lease(
        ...
        *,
        ...
        """Bind the live worker identity to the exact durable Attempt lineage."""
        ...
        return self.evidence.bind_lease(
    ...
    def release(
        ...
        *,
        ...
        return self._transition(
            ...
            idempotency_key=f"lease-release:{lease_id}:{self._stable_reason(reason)}",
            effect_state="EFFECT_NOT_EVALUATED",
        )
    ...
    def mark_stale(
        ...
        *,
        ...
        """Mark an active lease stale without inferring anything about external effects."""
        ...
        return self._transition(
            ...
            lease_id=lease_id,
            new_state="STALE",
            operation_kind="WORKER_LEASE_STALE",
    ...
    def mark_ambiguous(
        ...
        *,
        ...
        return self._transition(
            ...
            lease_id=lease_id,
            new_state="AMBIGUOUS",
            operation_kind="WORKER_LEASE_AMBIGUOUS",
            idempotency_key=idempotency_key,
    ...
    def reclaim(
        ...
        *,
        ...
        return self._transition(
            ...
            lease_id=lease_id,
            new_state="RECLAIMED",
            operation_kind="WORKER_LEASE_RECLAIM",
            idempotency_key=idempotency_key,
    ...
    def _transition(
        ...
        *,
        ...
        with self.state._transaction() as conn:
            ...
            attempt = self.state._require_attempt(
                conn,
                project_id,
                run_id,
                lease["phase_id"],
            ...
            if new_state != current and new_state not in {
                "RELEASED": {"ACTIVE"},
                "STALE": {"ACTIVE"},
                "AMBIGUOUS": {"ACTIVE", "STALE"},
                "RECLAIMED": {"STALE", "AMBIGUOUS"},
            }.get(new_state, set()):
            ...
            self.state._append_event_tx(
                ...
                event_type="RECOVERY_OPERATION_STARTED",
            ...
            next_sequence = sequence + 1
            conn.execute(
                "UPDATE lease_bindings SET state=?, released_at=? WHERE lease_id=?",
                (new_state, now if new_state in {"RELEASED", "STALE", "AMBIGUOUS", "RECLAIMED"} else lease["released_at"], lease_id),
            )
            self.state._append_event_tx(
                conn,
                project_id=project_id,
            ...
            if attempt_state != str(attempt["state"]):
                ...
                next_sequence = sequence + 1
                conn.execute(
                    "UPDATE attempts SET state=? WHERE project_id=? AND run_id=? AND phase_id=? AND task_id=? AND attempt_id=?",
                    (attempt_state, project_id, run_id, lease["phase_id"], lease["task_id"], lease["attempt_id"]),
                )
                self.state._append_event_tx(
                    conn,
                    project_id=project_id,
                    run_id=run_id,
                    sequence=next_sequence,
                    event_type=f"ATTEMPT_{attempt_state}",
                    entity_type="attempt",
                    entity_id=str(lease["attempt_id"]),
                    payload={"task_id": lease["task_id"], "lease_id": lease_id, "lease_state": new_state},
                )
                sequence = next_sequence
            ...
            if task_state != str(task["state"]):
                ...
                next_sequence = sequence + 1
                conn.execute(
                    "UPDATE tasks SET state=?, completed_at=NULL WHERE project_id=? AND run_id=? AND phase_id=? AND task_id=?",
                    (task_state, project_id, run_id, lease["phase_id"], lease["task_id"]),
                )
                self.state._append_event_tx(
                    conn,
                    project_id=project_id,
                    run_id=run_id,
                    sequence=next_sequence,
                    event_type=f"TASK_{task_state}",
                    entity_type="task",
                    entity_id=str(lease["task_id"]),
                    payload={"phase_id": lease["phase_id"], "attempt_id": lease["attempt_id"], "lease_id": lease_id},
                )
                sequence = next_sequence
            ...
            next_sequence = sequence + 1
            conn.execute(
                "UPDATE recovery_operations SET state=?, completed_at=?, effect_reference=?, effect_hash=? WHERE operation_id=?",
                (terminal_operation_state, now if terminal_operation_state == "COMMITTED" else None, effect_reference, effect_hash, operation_id),
            )
            self.state._append_event_tx(
                conn,
                project_id=project_id,
                run_id=run_id,
                sequence=next_sequence,
                event_type=f"RECOVERY_OPERATION_{terminal_operation_state}",
                entity_type="recovery_operation",
                entity_id=operation_id,
                payload={
                    "operation_kind": operation_kind,
            ...
            return self._result(
                dict(operation),
                dict(lease) | {"state": new_state, "released_at": now if new_state != "ACTIVE" else lease["released_at"]},
                dict(attempt) | {"state": attempt_state},
                dict(task) | {"state": task_state},
                effect_state,
            )
    ...
    def _get_or_create_operation(
        ...
        *,
        ...
        conn.execute(
            "INSERT INTO recovery_operations(operation_id, idempotency_key, project_id, run_id, phase_id, task_id, attempt_id, operation_kind, state, started_at, completed_at, effect_reference, effect_hash) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (operation_id, idempotency_key, project_id, run_id, phase_id, task_id, attempt_id, operation_kind, "PLANNED", None, None, None, None),
        )
    ...
    def _validate_attempt_transition(current: str, new: str) -> None:
        allowed = {
            "STARTED": {"INTERRUPTED", "AMBIGUOUS", "RECOVERY_REQUIRED", "VALIDATED", "FAILED", "FINISHED"},
            "INTERRUPTED": {"AMBIGUOUS", "RECOVERY_REQUIRED"},
    ...
    def _validate_task_transition(current: str, new: str) -> None:
        allowed = {
            "PENDING": {"IN_PROGRESS", "INTERRUPTED", "RECOVERY_REQUIRED", "SAFE_STOP"},
    ...
    def _result(operation: Any, lease: Any, attempt: Any, task: Any, effect_state: str) -> LeaseRecoveryResult:
        return LeaseRecoveryResult(
            ...
            attempt_state=str(attempt["state"]),
            task_state=str(task["state"]),
            recovery_state=str(operation["state"]),
            effect_state=effect_state,
        )
