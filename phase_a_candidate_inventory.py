...
def _models_for_connection(
    ...
    if provider == "openrouter":
        ...
        if not isinstance(leader, Mapping):
            return ()
        models: list[str] = []
        if connection_id in leader.get("primary_pool", []):
            model = leader.get("primary_model")
            if isinstance(model, str) and model:
                models.append(model)
        if connection_id in leader.get("failover_pool", []):
...
def discover_connection_inventory(
    *,
    ...
    for connection_id in sorted(connections, key=_natural_id):
        ...
        configured_models = _models_for_connection(connection_id, provider, authoritative)
